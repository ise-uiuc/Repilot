...
def _load_rectify_fixes():
    ...
    data = pandas.read_csv("csv/rectify-correct-dfj2-single-line.csv", header=0)
    correct_fixes_dfj2 = set(data["Correct"].dropna().tolist())

    return correct_fixes, correct_fixes_dfj2


RECTIFY_D4J1_CORRECT_FIXES = rq_utils.get_correct_fixes(
    Path("data/correct-patches/rq1/d4j1-codet5-template-rectify")
)
RECTIFY_D4J2_CORRECT_FIXES = rq_utils.get_correct_fixes(
    Path("data/correct-patches/rq1/d4j2-codet5-template-rectify")
)
...
def get_all_fixes() -> dict[str, tuple[set[str], set[str]]]:
    ...
    all_fixes[rq_utils.TOOL_NAME] = (
        RECTIFY_D4J1_CORRECT_FIXES,
        RECTIFY_D4J2_CORRECT_FIXES,
    )
    return {
...
def check_overlap():
    correct_fixes, correct_fixes_dfj2 = _load_rectify_fixes()
